...
config = {
    'SECRET_CAPTCHA_KEY': f'{os.getenv("SECRET_CAPTCHA_KEY")}',
...
class CAPTCHA:
    ...
    def draw_lines(self, im, lines=25):
        ...
        for i in range(lines):
            if random.randint(0, 1) == 0:
                draw.line((random.randint(0, im.size[0]), 0, random.randint(0, im.size[0]), im.size[1],), width=2)
            else:
                draw.ellipse((
                    random.randint(-80, im.size[0]), random.randint(-80, -10), random.randint(-80, im.size[0]),
                    im.size[1] + random.randint(10, 80),), width=2)
        return im

    def create(self, length=None, digits=None):
        length = self.config['CAPTCHA_LENGTH']
        digits = self.config['CAPTCHA_DIGITS']
        size = 30
        ...
        background.paste(text_img, box=(
            random.randint(0, background.size[0] - text_size[0]), random.randint(0, background.size[1] - text_size[1])))
        ...
        c_hash = generate_password_hash(c_key, method=self.config['METHOD'], salt_length=8)
    ...
    def captcha_html(self, captcha):
        ...
        inpu = '<input type="text" class="simple-captcha-text"' + 'name="captcha-text">\n' + '<input type="hidden" name="captcha-hash" ' + 'value="%s">' % \
               captcha['hash']

        return '%s\n%s' % (img, inpu)

    def captcha_img(self, captcha):
        return f'data:image/png;base64, {captcha["img"]}'

    def captcha_input(self, captcha):
        inpu = '%s' % captcha['hash']
        return inpu

    def verify(self, c_text, c_hash, c_key=None):
        c_key = self.config['SECRET_CAPTCHA_KEY']
        c_text = c_text.upper()
    ...
    def init_app(self, app):
        ...
        app.jinja_env.globals.update(captcha_img=self.captcha_img)
        app.jinja_env.globals.update(captcha_input=self.captcha_input)
